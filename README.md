...
<!-- Fill in after running scripts/benchmark.js on your own hardware -->
...
| Witness generation time | -                                    | not separately reported                      |
| Proving time            | -                                    | 2.1s ± 0.15s (server-side, delegated)        |
| Verification time       | -                                    | 2.04ms (Apple M1 Pro)                        |
| Proof size              | -                                    | ~1300 bytes (base64, full zkLogin signature) |

Raw measurements: `build/benchmark_results.csv` (N runs, one row per run;
generated by `node scripts/benchmark.js N`).

**Interpretation:** the constraint count drop (~130x) is expected since
Poseidon and EdDSA are designed to be arithmetic-circuit-friendly, while
SHA-256 and RSA modular exponentiation are not — this directly confirms
the paper's own claim that these are the dominant cost in their circuit
(Section 5.1.1).
